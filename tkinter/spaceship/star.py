...
def main():
    ...
    canvas.create_image(240, bg_pos+320, image=img_bg, tag="SCREEN")
    if index == 0:
        canvas.create_text(240, 240, text="METOR",
                           fill="gold", font=fnt2, tag="SCREEN")
        canvas.create_text(240, 480, text="pressspace",
                           fill="lime", font=fnt1, tag="SCREEN")
        if key == "space":
            score = 0
            px = 240
            init_enemy()
    ...
    if index == 2:
        move_enemy()
        canvas.create_text(240, timer*4, text="GAMEOVER",
                           fill="red", font=fnt2, tag="SCREEN")
        if timer == 60:
            index = 0
            timer = 0
    canvas.create_text(240, 30, text="SCORE"+str(score),
                       fill="white", font=fnt1, tag="SCREEN")
    if koff == True:
        key = ""
        koff = False
    root.after(50, main)
